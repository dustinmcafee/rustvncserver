...
impl VncServer {
            ...
            match listener.accept().await {
                Ok((stream, addr)) => {
                    ...
                    if client_id_raw == 0 || client_id_raw >= u64::MAX - 1000 {
                        error!(
                            "Client ID counter overflow, rejecting connection from {addr}"
                        );
                        continue;
                    }
    ...
    #[must_use] pub fn framebuffer(&self) -> &Framebuffer {
        &self.framebuffer
    }
            ...
            match connection_result {
                Ok(stream) => {
                    ...
                    match client_result {
                        ...
                        Err(e) => {
                            error!(
                                "Failed to initialize VNC client for reverse connection: {e}"
                            );
                        }
                    }
                }
            ...
            match connection_result {
                Ok(client) => {
                    ...
                    let msg_handle = tokio::spawn(async move {
                        ...
                        if let Err(e) = result {
                            error!(
                                "Repeater client {client_id} message handling error: {e}"
                            );
                        }
                    });
